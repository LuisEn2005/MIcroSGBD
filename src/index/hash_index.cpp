...
Status HashIndex::Insert(
    ...
    while (current_page_id != INVALID_PAGE_ID) {
        ...
        bool inserted = false;
        status = bucket.Insert(key, value, &inserted);

        if (status.ok()) {
            if (!bpm_->UnpinPage(current_page_id, inserted)) {
                return Status::IOError(
                    "Could not unpin hash bucket after insert"
                );
            }
        ...
        if (!bpm_->UnpinPage(current_page_id, false)) {
            return Status::IOError(
                "Could not unpin full hash bucket"
            );
        }

        if (status.code() != StatusCode::OUT_OF_MEMORY) {
            return status;
        }

        if (overflow_page_id == INVALID_PAGE_ID) {
